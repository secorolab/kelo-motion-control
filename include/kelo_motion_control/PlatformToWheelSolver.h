...
#include "kelo_motion_control/SmartWheelKinematics.h"
  ...
  /**
   * @brief function which takes pivot angles and platform forces as input and calculates the
   * required wheel torques by suitably calling individual functinalities
   *
   * @param wheel_torques 1-D array of shape (8,) which is a placeholder for calculated wheel
   * torques for each wheel unit
   * @param pivot_angles 1D array of shape (4,) that stores current pivot angles of individual pair
   * of wheels
   * @param b platform force matrix of shape 3x1
   * @param b_verify platform force matrix of shape 3x1, which is calculated from pivot forces to
   * verify results
   * @param A Jacobian matrix of shape 3x8, which is further used to store W.dot(A).dot(K)
   * @param A_inv_T initially store left singular matrix of SVD decomposition of A and later it
   * stores the inverse of W.dot(A).dot(K)
   ...
   * @param u a vector consisting of singular values of Sigma matrix in SVD decomposition of
   * A(Jacobian matrix)
