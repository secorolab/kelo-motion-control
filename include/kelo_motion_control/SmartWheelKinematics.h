...
extern "C"
{
  ...
  void map_pivot_force_to_wheel_torque(const double *pivot_forces, double *wheel_torques,
                                       const double radius, const double castor_offset,
                                       const double half_wheel_distance);
  /**
   * @brief pivot forces for every pair of wheels are provide as input to
   * "map_pivot_force_to_wheel_torque" to calculate corresponding torque values
   *
   * @param pivot_forces forces at individual wheel units
   * @param wheel_torques 1-D array of shape (8,) which is a placeholder for calculated wheel
   * torques for each wheel unit
  ...
  void map_pivot_forces_to_wheel_torques(const double *pivot_forces, double *wheel_torques,
                                         const double radius, const double castor_offset,
                                         const double half_wheel_distance);
